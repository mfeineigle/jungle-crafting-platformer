...
func _check_blocked_sides():
	for seg in segments:
		if seg.is_side_blocked(seg.Side.FRONT):
			_disable_side_all_segments(seg.Side.FRONT)
		if seg.is_side_blocked(seg.Side.BACK):
			_disable_side_all_segments(seg.Side.BACK)
