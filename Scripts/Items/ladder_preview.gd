...
@export var ladder_segment_scene: PackedScene
...
func _ready():
	buildable_prototype = ladder.instantiate() as Buildable
	update_preview()
...
func assemble_segments():
	var final_ladder = ladder.instantiate()
	final_ladder.current_height = current_height
	return final_ladder
...
func _unhandled_input(event):
	...
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_WHEEL_UP and event.pressed:
			current_height += 1
			update_preview()
		elif event.button_index == MOUSE_BUTTON_WHEEL_DOWN and event.pressed and current_height > min_segments:
			current_height -= 1
			update_preview()
...
func update_preview():
	...
	for i in range(current_height):
		var seg = ladder_segment_scene.instantiate()
