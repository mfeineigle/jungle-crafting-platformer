...
@export var component_control: bool = false
func _ready() -> void:
	if not component_control: # Wind singleton control
	...
	for mesh in mesh_nodes:
		...
		var mat = preload("uid://ctwjsuabouwwl").duplicate(true) as ShaderMaterial
...
# Called via signal from Wind singleton
# or directly via windComponent
